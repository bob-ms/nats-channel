 ...
 *
 * Registration metadata stamps `context_id` via a lazy per-session mapping
 * read (registration.ts) — see design-of-record
 * proposed/bob-ms/skills/design-fleet-hub-session-identity.md §3.
 */
...
// ── Config types ───────────────────────────────────────────────────────
// This is the static per-user config surface only — `context`, `owner`,
// `sessionName`, `permissions`. It carries no per-session fact (AC #4);
// `context_id` is resolved separately via a lazy mapping read, keyed by
// `CLAUDE_CODE_SESSION_ID` (see registration.ts).
...
async function resolveSessionName(nc: NatsConnection, base: string, owner: string): Promise<string> {
  ...
  const client = svcm.client({ strategy: 'count', maxWait: 1000, maxMessages: 50 })
...
// ── Attachment staging ─────────────────────────────────────────────────
// Request envelope parsing goes through the SDK's `decodeEnvelope` (handles
// §5.1/§5.2/§5.3 + strict base64 + filename safety in one place). The local
// `{filename, bytes}` shape is kept here only because the staging helper
// writes the bytes verbatim — the adapter at the decode boundary is one map().
  ...
  for await (const s of nc.status()) {
    switch (s.type) {
      ...
      case 'reconnect':
        ...
        break
      case 'error':
        process.stderr.write(`nats channel: connection error: ${s.error.message}\n`)
        break
    }
  }
})()
