...
- `config.json` never carries the per-session `context_id` — that's a
  separate lazy read keyed by `CLAUDE_CODE_SESSION_ID` (see the component
  README's "Session identity" section).
